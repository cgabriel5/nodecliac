...
module.exports = (contents, commandname, source) => {
	...
	let issue = (result, type = "error") => {
		...
		let linenum = result.line;
		let linechar = result.index;
		...
		if (!line_type) {
			...
			if (currentflag) {
				...
				if (!r_start_line_char.test(char)) {
					error(char, 0, void 0, indentation.length);
				}
				...
				if (char === "@") {
					...
					if (!currentchain) {
						...
						if (/^-[ \t]/.test(chars_str)) {
							error(void 0, 3, void 0, 0);
						}
						// Flag is improperly nested.
						else if (/^-{1,2}/) {
							error(void 0, 4, void 0, 0);
						}

						// General error.
						error(void 0, 5, void 0, 0);
					}
				} else if (char === ")") {
					line_type = "close_parenthesis";
				}
