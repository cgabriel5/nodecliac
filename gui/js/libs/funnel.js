...
(function (window) {
	...
	var library = (function () {
		...
		var helper_one = function (this_, type, args) {
			var elements,
				...
				input = function (args) {
					...
					for (
						...
						current_item = args[i];
						current_item.charCodeAt(0) !== 33
							? has.push(current_item)
							: not.push(current_item.substring(1));
					}
					return {
						has: has,
						not: not
				...
				has = function (elements, has_not, filter, reverse) {
					for (
						...
						if (filter(current_element, has_not, reverse))
							filtered.push(current_element);
					}
					return filtered;
				},
				filters = {
					...
					text: function (element, has_not, reverse) {
						for (
							var current_text, i = 0, l = has_not.length;
							i < l;
							i++
						) {
					...
					classes: function (element, has_not, reverse) {
						for (
							var current_class, i = 0, l = has_not.length;
							i < l;
							i++
						) {
			...
			// filter elements
			if (cleaned_input.has.length)
				elements = has(array, cleaned_input.has, filters[type]);
			if (cleaned_input.not.length)
				elements = has(
					elements || array,
					cleaned_input.not,
					filters[type],
		...
		var Library = class__({
			...
			constructor__: function () {
				...
				var is_recursive =
					args[0] === true && dtype(args[1], "arguments");
					...
					if (data_type === "string") {
						...
						// else apply the filter and add all returned (filtered) elements to array
							...
							elements = elements.concat(
								to_array(this[parts[1]]([cid]))
							); // i.e. -> this.all()
					} else if (/^(html|text|comment)/.test(data_type)) {
			...
			methods__: {
				...
				all: function (source) {
					...
					var elements = [],
						...
						array = source
							? source
							: this_.stack[this_.stack.length - 1];
					// loop through source and get all its elements
					for (var i = 0; i < l; i++) {
						elements = elements.concat(
							to_array(array[i].getElementsByTagName("*"))
						);
					}
				...
				textNodes: function (source) {
					...
					for (var current_element, child_nodes, i = 0; i < l; i++) {
						...
						for (var j = 0, ll = child_nodes.length; j < ll; j++) {
							if (
								child_nodes[j].nodeType === 3 &&
								child_nodes[j].textContent.trim().length
							)
								elements.push(child_nodes[j]);
						}
					}
					// only returns for constructor
					if (source) return elements;
				...
				parents: function (source) {
					...
					for (var current_element, i = 0; i < l; i++) {
						...
						while (current_element) {
							...
							if (current_element && current_element.tagName) {
				...
				siblings: function (source) {
					...
					for (
						var first_element, current_element, i = 0;
						i < l;
						i++
					) {
						current_element = array[i];
				...
				attrs: function () {
					...
					var filters = {
							...
							"$=": function (pav, value) {
								return (
									value.length - value.lastIndexOf(pav) ===
									pav.length
								);
							},
							/**
						...
						set = function (elements, attrs) {
							...
							var screened = [];
							loop1: for (
								var current_element, i = 0, l = elements.length;
								i < l;
								i++
							) {
								current_element = elements[i];
								for (
									var current_attr, j = 0, ll = attrs.length;
									j < ll;
									j++
								) {
						...
						input = function (attrs) {
							...
							var types = Object.keys(filters),
								...
								screened = [];
							loop1: for (
								var current_attr, i = 0, l = attrs.length;
								i < l;
								i++
							) {
								...
								for (var j = ll - 1; j > -1; j--) {
									...
									if (check) {
										...
										if (current_attr.charCodeAt(0) === 33)
											screened.push([
												false,
												"!",
												current_attr.substring(1)
											]);
				...
				state: function (property, state) {
					...
					var filter =
						states[property] ||
						function (element, bool, property) {
					...
					for (
						var current_element, i = 0, l = array.length;
						i < l;
						i++
					) {
						current_element = array[i];
						if (filter(current_element, state, property))
							elements.push(current_element);
					}
					// add elements to the object
					this_.stack.push(elements);
				...
				only: function (wanted_indices) {
					...
					for (
						...
						if (current_windex < l)
							elements.push(
								current_windex < 0
									? array[l + current_windex]
									: array[current_windex]
							);
					}
					// add elements to the object
					this_.stack.push(elements);
