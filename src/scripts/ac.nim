...
# Get passed in CLI arguments count.
# echo os.paramCount()
...
var usedflags = initTable[string, string]
var usedflags_valueless = initTable[string, string]
var usedflags_multi = initTable[string, string]
...
proc fn_paramparse(input: var string): seq =
...
proc fn_set_envs() =
    ...
    var envs = {
        ...
        "${prefix}COMP_POINT": intToStr(cpoint), # Caret index when [tab] key was pressed.
        ...
        "${prefix}COMP_LINE_LENGTH": intToStr(cline_length), # Original input's length.
        "${prefix}INPUT_LINE_LENGTH": intToStr(cline_length), # CLI input from start to caret index string length.
        "${prefix}ARG_COUNT": intToStr(l), # Amount arguments parsed before caret position/index.
...
proc fn_execute_command(command_str: string , flags: seq, last_fkey: string) =
    # Cache captured string command.
    var arguments = fn_paramparse(command_str)
    ...
    if args_count > 1:
        ...
        for i in countup(1, args_count - 1, 1):
            ...
            if arg.startsWith('$'):
                ...
                let res = os.execShellCmd(cmdarg)
    ...
    # Run the command.
    var lines = os.execShellCmd(command)
...
proc fn_parser() =
    ...
    # echo "command:\n format setup make print registry uninstall status"
    # echo "command:\n uninstall"
...
proc fn_printer() =
    ...
    completions = map(completions, proc (x: string): string =
        ...
        # (i.e. --flag="some-word...).
      let final_space = if isflag_type and x.endsWith('=') and x.find({'"', '\''}) != 0 and nextchar == "": " " else: ""
    ...
    if commandchain == "":
        ...
        # For first level flags...
            ...
            if matches.len > 0:
                ...
                db_dict['_']["_"].add("flags", @[matches[0]])
        ...
        for lne in acdef.split("\n"):
            ...
            if ord(line[0]) == 45:
                ...
                if not db_dict.hasKey(fchar):
                    var fletter = fn_rm_firstchar(chain)
                    db_dict[fchar] = initTable[string, Table[string, seq[string]]]()
                    db_dict[fchar][fletter] = initTable[string, seq[string]]()
                    db_dict[fchar][fletter].add("commands", commands)
                    db_dict[fchar][fletter].add("flags", @[line])
            else:
                # Store fallback.
